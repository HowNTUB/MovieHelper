    ...
    for movieInfo in movieList[(int(page)-1)*10:int(page)*10]:
        ...
        for movietime in movieInfo.select("ul + ul li")[:-1]:
            ...
            if len(movietime.text) >10:
                print("NO")
            elif movietime.text.split() == movieName:
                print("NO")
            elif movietime.text.split()[:2] == "片長":
                print("NO")
            ...
            elif movietime.text.replace("：",":") > time.strftime("%H:%M", time.localtime(time.time()+28800)) and (movietime.text[-1] == "0" or movietime.text[-1] == "5"):
                timeContents.append({
                    "type": "box",
                    "layout": "vertical",
                    "margin": "md",
...
def use_movieurl_get_movieMoment(movieID, inAreaID, page):
    ...
    for content in movietheaterData[(int(page)-1)*10:int(page)*10]:
        ...
        for movietime in [i for i in content.select("li")][2:]:
            ...
            print(movietime.text.replace("：",":") > time.strftime("%H:%M", time.localtime(time.time()+28800)))
